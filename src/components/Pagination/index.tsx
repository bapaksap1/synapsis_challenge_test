...
const Pagination = <T extends {}>({ data, itemsPerPage, renderItem }: PaginationProps<T>) => {
    ...
    const renderFirstPageButton = () => (
        <button onClick={() => handleClick(1)} className="px-4 py-2 rounded-md">
            First
        </button>
    );

    const renderLastPageButton = () => (
        <button onClick={() => handleClick(totalPages)} className="px-4 py-2 rounded-md">
            Last
        </button>
    );
    ...
    const renderPaginationButtons = () => {
        ...
        if (currentPage > 1) {
            buttons.push(
                <button onClick={() => handleClick(currentPage - 1)} className="px-4 py-2 rounded-md">
                    Prev
                </button>
            );
        }
